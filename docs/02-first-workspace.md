...
1. Create a copy of your existing workspace from the RunAI web portal.
2. Replace the GitHub URL in the runtime args with one of your own
   repos (use the `archive/refs/heads/<branch>.tar.gz` form).
3. Start the workspace and confirm `/work/repo` shows up
   in Jupyter.

Once you've done that successfully, you can build any new workload
by editing `runtime args + image + GPU + Data Sources` as needed.
...
convert this Qwen2.5-7B example into that pattern.
